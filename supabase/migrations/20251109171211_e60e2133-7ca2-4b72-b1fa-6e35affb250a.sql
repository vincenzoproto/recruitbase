...
DROP POLICY IF EXISTS "Everyone can view recruiter stats for leaderboard" ON public.recruiter_stats;
DROP POLICY IF EXISTS "Users can update their own stats" ON public.recruiter_stats;
DROP POLICY IF EXISTS "System can insert stats" ON public.recruiter_stats;
DROP POLICY IF EXISTS "Everyone can view academy content" ON public.academy_content;
DROP POLICY IF EXISTS "Users can view their own metrics" ON public.daily_recruiter_metrics;
DROP POLICY IF EXISTS "System can insert metrics" ON public.daily_recruiter_metrics;
DROP POLICY IF EXISTS "System can update metrics" ON public.daily_recruiter_metrics;
...
-- Insert sample academy content (skip if already exists)
INSERT INTO public.academy_content (title, category, content_type, description, article_text, duration_minutes, order_position)
SELECT * FROM (VALUES
...
) AS v(title, category, content_type, description, article_text, duration_minutes, order_position)
WHERE NOT EXISTS (SELECT 1 FROM public.academy_content WHERE category = v.category);